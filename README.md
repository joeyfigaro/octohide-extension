...
```bash
pnpm dev            # Chrome, live reload
pnpm dev:firefox    # Firefox, live reload
pnpm test           # run tests (vitest)
pnpm compile        # generate types + tsc --noEmit
pnpm build          # production build, Chrome
pnpm build:firefox  # production build, Firefox
```

In non-interactive shells, prefix commands with `CI=true` to skip WXT's interactive dependency-check prompt (e.g. `CI=true pnpm build`).
